...
12. Wiederholung „Batteriewarnung" legt eine automatische Sammel-Aufgabe an: statt an einen einzelnen Sensor gebunden zu sein, überwacht sie *alle* Batterie-Entities, die Home Assistant kennt (`sensor`/`binary_sensor` mit `device_class: battery`), und wird fällig, sobald mindestens eine davon ihren Warn-Schwellenwert erreicht oder unterschreitet - inklusive Liste der betroffenen Batterien (Name + Stand) direkt an der Aufgabe. Der admin-only Kartenabschnitt „Batterien" (unterhalb der Aufgabenliste) listet jede erkannte Batterie und erlaubt es, sie einzeln von der Überwachung auszuschließen oder ihr eine eigene Warnschwelle zu geben - beides über die neue Websocket-API `family_tasks/battery_override/*`. Der Standard-Schwellenwert (20 %, sofern nicht geändert) wird über die Integrations-Optionen (Einstellungen → Geräte & Dienste → Family Tasks → Konfigurieren) festgelegt.
